...
enum BaseDirectory {
  ///As returned by getDownloadsDirectory()
  downloads,
...
class Request {
  ...
  Request.fromJsonMap(Map<String, dynamic> jsonMap)
      : url = jsonMap['url'] ?? '',
        ...
        httpRequestMethod =
            jsonMap['httpRequestMethod'] as String? ?? (jsonMap['post'] == null ? 'GET' : 'POST'),
        ...
        creationTime =
            DateTime.fromMillisecondsSinceEpoch((jsonMap['creationTime'] as num?)?.toInt() ?? 0);
...
abstract class Task extends Request {
    ...
    switch (baseDirectory) {
      ...
      case BaseDirectory.applicationLibrary:
        baseDir = Platform.isMacOS || Platform.isIOS
            ...
            : Directory(path.join((await getApplicationSupportDirectory()).path, 'Library'));
        break;
      case BaseDirectory.downloads:
        baseDir = await _getDownloadDirectory();
        break;
    }
  ...
  Future<Directory> _getDownloadDirectory() async {
    ...
    if (directory == null) throw Exception("Method get Downloads Directory must return not null");
    return directory;
  }
  ...
  Task.fromJsonMap(Map<String, dynamic> jsonMap)
      : taskId = jsonMap['taskId'] ?? '',
        ...
        baseDirectory = BaseDirectory.values[(jsonMap['baseDirectory'] as num?)?.toInt() ?? 0],
    ...
    try {
      ...
      if ([200, 201, 202, 203, 204, 205, 206].contains(response.statusCode)) {
        final disposition = response.headers.entries
            .firstWhere((element) => element.key.toLowerCase() == 'content-disposition')
            .value;
        // Try filename="filename"
        final plainFilenameRegEx = RegExp(r'filename="?([^"]+)"?.*$', caseSensitive: false);
        ...
        if (match != null &&
          ...
          try {
            final suggestedFilename =
                match.group(1) == 'UTF-8' ? Uri.decodeComponent(match.group(3)!) : match.group(3)!;
...
String _urlWithQueryParameters(String url, Map<String, String>? urlQueryParameters) {
...
class TaskUpdate {
  ...
  TaskUpdate.fromJsonMap(Map<String, dynamic> jsonMap) : task = Task.createFromJsonMap(jsonMap);
...
class TaskStatusUpdate extends TaskUpdate {
  ...
  TaskStatusUpdate.fromJsonMap(Map<String, dynamic> jsonMap)
      : status = TaskStatus.values[(jsonMap['taskStatus'] as num?)?.toInt() ?? 0],
        exception =
            jsonMap['exception'] != null ? TaskException.fromJsonMap(jsonMap['exception']) : null,
        super.fromJsonMap(jsonMap);

  /// Return JSON Map representing object
  @override
  Map<String, dynamic> toJsonMap() =>
      {...super.toJsonMap(), 'taskStatus': status.index, 'exception': exception?.toJsonMap()};
}
...
class TaskProgressUpdate extends TaskUpdate {
  ...
  /// Return JSON Map representing object
  @override
  Map<String, dynamic> toJsonMap() => {...super.toJsonMap(), 'progress': progress};
}
...
class ResumeData {
  ...
  ResumeData.fromJsonMap(Map<String, dynamic> jsonMap)
      : task = Task.createFromJsonMap(jsonMap['task']),
        ...
        requiredStartByte = (jsonMap['requiredStartByte'] as num?)?.toInt() ?? 0;

  /// Return JSON Map representing object
  Map<String, dynamic> toJsonMap() =>
      {'task': task.toJsonMap(), 'data': data, 'requiredStartByte': requiredStartByte};
